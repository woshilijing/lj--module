(function () {
    ...
    //->tarObj:存储的是当前元素运动的目标位置的样式集合
    // ->{left:1000,width:200,opacity:1...}
    //->duration:当前运动的总时间
    //->effect:运动的效果(匀速、加速、减速...)
    //->callBack:回调函数,当动画运动完成后需要做的事情都写在这个函数中
    function animate(curEle, tarObj, duration, effect, callBack) {
        var tempEffect = Effect.Linear;//首先设置默认值
        if (typeof effect === "number") {
            //->传递进来的是一个数字,分别代表不同的运动方式：1->Linear
            // 2->Elastic-easeOut 3->Back-easeOut 4->Bounce-easeOut 5->Expo-easeIn
            var ary = ["Linear", "Elastic-easeOut",
                "Back-easeOut", "Bounce-easeOut", "Expo-easeIn"];
            for (var i = 0; i < ary.length; i++) {
                if ((i + 1) === effect) {
                    var curAry = ary[i].split("-");
                    if (curAry.length === 1) {
                        tempEffect = Effect[curAry[0]];
                    } else {
                        tempEffect = Effect[curAry[0]][curAry[1]];
                    }
                    break;
                }
            }
            ...
            tempEffect = effect.length === 1 ? Effect[effect[0]]
                : Effect[effect[0]][effect[1]];
        ...
        var timer = window.setInterval(function () {
            ...
            //->我们走的时间已经大于等于总时间,说明到达目标位置了,
            // 我们让当前元素的样式等于目标位置的值,并且执行回调函数
            if (time >= duration) {
                for (var key in tarObj) {
                    if (tarObj.hasOwnProperty(key)) {
